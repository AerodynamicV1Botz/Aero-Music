...
from . import helps, paginate_module, modules, load_modules
...
async def check_duration(chat_id, date_time, cb):
    ...
    duration_limit = int((await chat_db.get_chat(chat_id))["duration"])
    duration_limit = duration_limit * 60
    if duration >= duration_limit:
        return await cb.answer(
            await gm(chat_id, "duration_reach_limit", [str(timedelta(seconds=duration_limit))]),
            show_alert=True
        )
    pass
...
async def _close_button(_, cb: CallbackQuery):
    ...
    if cb.from_user.id != user_id or not user_id:
        return await cb.answer(await gm(cb.message.chat.id, "not_for_you"), show_alert=True)
    if modules:
        modules.clear()
...
async def goback(client: Client, hee: CallbackQuery):
    ...
    await hee.edit_message_text(
        await gm(chid, "pm_greet", [str(hee.message.from_user.mention)]),
                ...
                [
                    [
                        InlineKeyboardButton(
                            await gm(chat_id, "commands"), callback_data=f"plug_back|{user_id}"
                        ),
                        InlineKeyboardButton(
